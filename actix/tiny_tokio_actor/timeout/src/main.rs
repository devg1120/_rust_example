...
// Mark the struct as an actor and set a timeout.
// This actor will stop after 500ms of not receiving
// any message
...
pub async fn main() -> Result<(), ActorError> {
    ...
    let response = actor_ref.ask(msg.clone()).await.unwrap();
    //assert_eq!(response, "Ping!".to_string());

    // Wait shorter than the timeout
